 ...
 * @author Tatu Lund
 *
 * @param <T> Bean type used in TreeData
 */
public class TreeComboBox<T> extends Composite<HorizontalLayout> implements SingleSelect<TreeComboBox<T>, T>,
    ...
    public TreeComboBox(ValueProvider<T, String> valueProvider) {
		...
		filterField.addValueChangeListener(event -> {
			if (event.isFromClient() && !event.getValue().isEmpty()) {
				...
				dataProvider.setFilter(item -> {
					switch (filterMode) {
					case EXACT_CASE:
						return this.valueProvider.apply(item).equals(event.getValue());
					case STARTS_WITH_CASE:
						return this.valueProvider.apply(item).startsWith(event.getValue());						
					case CONTAINS_CASE:
						return this.valueProvider.apply(item).contains(event.getValue());						
					case EXACT:
						return this.valueProvider.apply(item).toLowerCase().equals(event.getValue().toLowerCase());						
					case STARTS_WITH:
						return this.valueProvider.apply(item).toLowerCase().startsWith(event.getValue().toLowerCase());						
					case CONTAINS:
						return this.valueProvider.apply(item).toLowerCase().contains(event.getValue().toLowerCase());						
					}
					return this.valueProvider.apply(item).toLowerCase().contains(event.getValue().toLowerCase());
				});
		...
		tree.asSingleSelect().addValueChangeListener(event -> {
			if (event.getValue() != null) {
				filterField.setValue(this.valueProvider.apply(event.getValue()));
				popup.setOpened(false);
				filterField.focus();
			}
		});

		Registration reg = filterField.addKeyDownListener(Key.ARROW_DOWN, event -> {
			popup.setOpened(true);
			tree.focus();
		});
	...
	private void selectFilteredItem(T item) {
		HierarchicalQuery<T, SerializablePredicate<T>> query = new HierarchicalQuery<>(dataProvider.getFilter(),item);
    ...
    /**
     * Constructs a new TreeComboBox Component with given caption and {@code TreeData}.
     *
     * @param valueProvider
     *            the item caption provider to use, not <code>null</code>
     * @param treeData
     ...
     */
    public TreeComboBox(HierarchicalDataProvider<T, ?> dataProvider, ValueProvider<T, String> valueProvider) {
    	this(valueProvider);
	...
	public Registration addValueChangeListener(
		...
		return tree.asSingleSelect().addValueChangeListener((ValueChangeListener<? super ComponentValueChangeEvent<Grid<T>, T>>) listener);
	}
     ...
     * removed.
     * <p>
     * Note: The content in popup needs an explicit width, hence "100%" does
     * not work.
     * 
     * @param width
